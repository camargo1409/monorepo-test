...
class RequestController {
    async index ({ request, response, auth }) {
        ...
        if(as === "customer"){
            const requests = await Request.query().where('customer_id',id).with('provider').fetch()
            return response.json(requests)
        }

        if(as === "provider"){
            const requests = await Request.query().where('post_provider_id',id).with('customer').fetch()
            return response.json(requests)
        }
    ...
    async store({request,response,auth}){
        ...
        if(customer_id === post_provider_id){
            return response.status(400).json({msg:"customer and provider are the same users"})
        }
    ...
    async update({request,response,auth,params}){
        ...
        if(action === "accept"){
            if(req.post_provider_id !== user_id){
                return response.status(401).json({msg:"you aren't the provider of the request"})
            }

            const {service_price} = request.post()

            if(!service_price) return response.status(400).json({msg:"provide the price of service"})
